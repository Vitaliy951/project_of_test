...
В этом проекте реализован магазин, который управляет продуктами, включая смартфоны и газонную траву. 

### Функциональность

- **Базовый абстрактный класс** `BaseProduct`: Определяет общие для всех продуктов методы.
- **Класс `Product`**: Является наследником `BaseProduct` и содержит общие атрибуты и методы для всех продуктов, включая возможность сложения цен.
- **Классы `Smartphone` и `LawnGrass`**: Наследуются от класса `Product` и добавляют специфические атрибуты.
- **Класс-миксин `LoggingMixin`**: При создании объекта выводит информацию о параметрах.
- **Класс `Category`**: Позволяет группировать продукты и добавлять новые.

### Тестирование

Проект включает тесты, написанные с использованием `pytest`, которые проверяют создание продуктов, добавление в категории и обработку ошибок. Покрытие тестами составляет более 75%.

### Установка и запуск

1. Клонируйте репозиторий.
2. Установите зависимости (если есть).
3. Запустите `main.py` для работы с продуктами.
4. Запустите `test_main.py` для выполнения тестов.
